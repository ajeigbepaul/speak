...
const RenderPost = React.memo(({ item, setEditingPost }: RenderPostProps) => {
    ...
    <View className="p-4 mb-4 bg-white rounded-lg shadow-sm">
      ...
      {item.updatedAt && (
        <Text className="text-xs text-gray-400 mt-2">
          Last updated: {useFormattedDate(item.updatedAt)}
        </Text>
      )}
    </View>
  );
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50 p-4">
        ...
        <FlashList
          ...
          renderItem={({ item }) => (
            <PostErrorBoundary>
              <RenderPost item={item} setEditingPost={setEditingPost} />
            </PostErrorBoundary>
          )}
